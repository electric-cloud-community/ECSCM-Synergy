    ...
    extends ComponentBase
{
    ...
    @Override public Widget doInit()
    {
        ...
        Widget htmlH1 = new HTML("<h1>Synergy Changelog</h1>");

        vPanel.add(htmlH1);

        Widget htmlLabel = new HTML(
                "<p><b>Synergy changelogs associated with the ElectricCommander job:</b></p>");
            ...
            final FormTable formTable)
    {
        ...
        if (getLog().isDebugEnabled()) {
            getLog().debug(
                "Synergy Reports doInit: setting up getProperties command request");
        }
        ...
        if (getLog().isDebugEnabled()) {
            getLog().debug("Synergy Reports doInit: Issuing Commander request: "
